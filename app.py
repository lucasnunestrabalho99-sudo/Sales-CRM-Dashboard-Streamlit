...
st.set_page_config(page_title="Banco de Dados - Clientes", layout="wide")
...
# --- FUNÇÃO DE LEITURA DE DADOS (COM CACHE MANUAL) ---
...
def load_data():
    ...
    SELECT      ge.descricao            AS [Gerencia]       ,
                ...
                (SELECT o.Descricao FROM ped_vda join OrigemPedidoVenda o on ped_Vda.origem_pedido = o.OrigemPedidoVenda WHERE pedvdaid = ped.pedvdaid) AS OrigemUltPedido,
    ...
    LEFT JOIN   (   SELECT  cd_clien, max(pedvdaid) pedvdaid
                    FROM    ped_vda pv 
                    WHERE   dt_cad >= '20230101'
                    AND     pv.situacao = 'AB'
                    AND     pv.tp_ped in (SELECT tp_ped FROM tp_ped WHERE estat_com=1)
                    AND     pv.tp_ped not in ('VA','BD')
                    AND     pv.cd_vend not in ('FELIPEB','RODRIGO')
                    GROUP BY cd_clien       
                 ) AS ped ON cl.cd_clien = ped.cd_clien
    LEFT JOIN   (   SELECT      e.cd_clien, e.dt_encer, e.cd_usr_enc, r.descricao AS resultado, m.descricao AS motivo, l.texto AS obs
                    FROM    evento_tmkt e
                    JOIN    (   SELECT cd_clien, max(seq_evento) seq_evento FROM evento_tmkt WHERE situacao = 'EN' GROUP BY cd_clien) AS ev2 ON e.cd_clien = ev2.cd_clien AND e.seq_evento = ev2.seq_evento
                    JOIN    res_tmkt r ON e.cd_resultado = r.cd_resultado
                    LEFT JOIN   mot_res m ON e.cd_resultado = m.cd_resultado AND e.cd_motivo = m.cd_motivo
                    LEFT JOIN lin_txt l ON e.comentario = l.cd_texto AND l.num_lin = 1  ) UltContato ON cl.cd_clien = UltContato.cd_clien
    ...
    for df in [df_ativos]:
        ...
        # Garante numérico para formatação
    ...
    # --- IDENTIFICAÇÃO DE DUPLICADOS (COMPARTILHADOS) ---
...
if st.sidebar.button("🔄 Atualizar Dados do Banco", help="Força nova consulta ao SQL."):
    st.cache_data.clear()
    st.rerun()
...
try:
    ...
    # =========================================================================
    # LÓGICA DE AGRUPAMENTO DE VENDEDORES (APENAS CÓDIGOS PARA VISUALIZAÇÃO)
    # =========================================================================
    ...
    # =========================================================================
    # CRIAÇÃO DAS CÓPIAS PARA FILTRAGEM (Em Cascata)
    # =========================================================================
    df_a_filtered = df_ativos.copy()
    df_i_filtered = df_inativos.copy()

    # =========================================================================
    # SIDEBAR - FILTROS EM CASCATA (DINÂMICOS)
    # =========================================================================
    st.sidebar.markdown("---")
    st.sidebar.header("Filtros Analíticos")

    # 1. BUSCA POR TEXTO E VENDEDORES
    with st.sidebar.expander("🔍 Busca e Vendedores", expanded=True):
        ...
        # Filtros de Grupo e Coligação (Tipo Lista)
    ...
    # APLICAÇÃO DOS FILTROS DE TEXTO E LISTA
    for df in [df_a_filtered, df_i_filtered]:
    ...
    row1_c1.metric("Clientes Únicos", format_ptbr(total_clientes_unicos), help="Total de CNPJs/CPFs distintos (sem duplicidade).")
    row1_c2.metric("Total Posições (Linhas)", format_ptbr(total_linhas_ativos), help="Total de linhas na tabela.")
    row1_c3.metric("Compartilhados (Linhas)", format_ptbr(qtd_compartilhados), help="Ocorrências de clientes compartilhados.")
    ...
    with tab1:
        ...
        # O Excel recebe a base 100% filtrada, independente do botão de visualização
        excel_data_a = to_excel(df_a_filtered[cols_a_finais])
        st.download_button(label="📥 Baixar Excel Completo (Ativos)", data=excel_data_a, file_name=f"clientes_ativos_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    with tab2:
        ...
        st.download_button(label="📥 Baixar Excel Completo (Inativos)", data=excel_data_i, file_name=f"clientes_inativos_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        if not f_mostrar_todos and tot_ina_pos > 500:
            st.info(f"💡 Exibindo apenas os **Top 500** registros de {format_ptbr(tot_ina_pos)} para melhor performance. Use o menu lateral para carregar todos ou baixe o Excel.")
    ...
    with tab3:
        ...
        with col_map1:
            ...
            st.info("Use este controle para ajustar o tamanho dos pontos.")
    ...
    with tab4:
        ...
        st.info("Nota: A 'Última Venda Própria' considera apenas vendas com origem 'TARGET MOB'. Vendas de outras origens (DIGITAÇÃO, E-COMMERCE etc.) não contam como atividade do dono da rota.")
        
        lista_gerencias = sorted(df_rotas['Gerencia'].dropna().unique())
        
        padrao_desejado = [
            'GER INTERNO', 'MBR GER INT', 'MGR GER FARMA VET',
            'PET GER ESTRAT', 'SOR GER INTERNO', 'GER DIGITAL'
        ]
        ...
        if not df_rotas_analise.empty:
            ...
            st.download_button(label="📥 Baixar Excel Completo (Rotas)", data=excel_data_r, file_name=f"analise_rotas_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Bases_Cruzadas": st.column_config.TextColumn("Bases Cruzadas", help="Códigos de outros vendedores que atendem clientes desta rota")
                }
            )
        else:
            st.warning("Nenhuma rota encontrada com os filtros selecionados (Verifique se a Gerência foi excluída no filtro acima).")
